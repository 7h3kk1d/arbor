/* p17 REPL — drives the editing-context model by hand.
...
/* `:open <expr> as Name` — generative open of a module (a Sig-typed value).
   One n-ary `Open` mints a fresh witness-less Abstract per opaque component;
   every binding's name is recovered from the sig's own labels: `Name.t` per
   type component (opaque and manifest alike — the manifest binding is the
   translucency payoff), `Name` for the module, `Name.f` per value member. */
let cmd_open_module = rest =>
  switch (split_first(rest, " as ")) {
  | None => print_endline("usage: :open <expr> as <Name>")
  | Some((exprs, tail)) =>
    let modname = String.trim(tail);
    ...
    | Ok(se) =>
      ...
      | Error(e) => err(e)
      | Ok(node) =>
        switch (Open_module.open_package(st, mint_src, node)) {
        | Error(e) => err("open: " ++ e)
        | Ok({Open_module.type_bindings, module_hash, fields}) =>
          ...
          let bind_labeled = ((lh, h)) =>
            switch (label_leaf(lh)) {
            | Some(nm) => ignore(rebind(modname ++ "." ++ nm, h))
            | None => ()
            };
          ...
          let tlist =
            String.concat(
              ...
              List.filter_map(
                ((lh, _)) =>
                  Option.map(nm => modname ++ "." ++ nm, label_leaf(lh)),
                type_bindings,
              ),
            );
...
let handle = line => {
  ...
  if (line == "") {
    ...
    switch (split_first(rest, " as ")) {
    | Some(_) => cmd_open_module(rest) /* :open <expr> as Name */
...
let () = {
  print_endline("p17 — type abstraction REPL. :help for commands, :quit to exit.");
  let rec loop = () => {
    Printf.printf("p17> %!");
    switch (
