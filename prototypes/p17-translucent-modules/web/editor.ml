...
let view ~(state : State.t Bonsai.Value.t)
  ...
  (* The merged work area: type an expression, see its type + value live, then
     ...
     a module (its type is a sig), an "open as module" block appears that
     PREVIEWS every binding the open would create — names come from the sig's
     own labels, so there is nothing to fill in beyond the module name. *)
  let open_shape =
    match state.work_fb with State.Typed { open_shape; _ } -> open_shape | _ -> None
  in
  let open_block (shape : State.open_shape) =
    let comp_row ~kind (c : State.open_comp) =
      Vdom.Node.div
        ~attrs:[ Vdom.Attr.class_ "open-field-row" ]
        [
          Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "open-field-kind" ] [ Vdom.Node.text kind ];
          Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "chip chip-name" ] [ Vdom.Node.text ("." ^ c.cname) ];
          Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "open-field-type" ] [ Vdom.Node.text (" : " ^ c.cty) ];
        ]
    in
    let type_rows = List.map shape.type_comps ~f:(comp_row ~kind:"type") in
    let field_rows = List.map shape.fields ~f:(comp_row ~kind:"op") in
    Vdom.Node.div
      ~attrs:[ Vdom.Attr.class_ "open-block" ]
      ((Vdom.Node.div ~attrs:[ Vdom.Attr.class_ "detail-note" ]
          [ Vdom.Node.text "open as module — binds (under the name above):" ]
       :: type_rows)
      @ field_rows
      @ [
          Vdom.Node.button
            ~attrs:
              [
                ...
                Vdom.Attr.on_click (fun _ -> inject State.Open_module);
              ]
