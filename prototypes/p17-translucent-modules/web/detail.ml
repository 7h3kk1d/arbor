        ...
        let rows =
          ...
          | Some (Definition.Type (Tnode.Abstract mint)) ->
              [
                ...
                Vdom.Node.div
                  ~attrs:[ Vdom.Attr.class_ "detail-note" ]
                  [ Vdom.Node.text "opened from a module — no representation is visible (not even a hidden witness)" ];
              ]
          | Some (Definition.Type (Tnode.Sig comps)) ->
              (* a signature: list its components, opaque/manifest/value *)
              let leaf nm = match String.rsplit2 nm ~on:'.' with Some (_, l) -> l | None -> nm in
              let label_name lh =
                match Namespace.name_of s.ns lh with Some nm -> leaf nm | None -> Hash.short lh
              in
              let ranks = Tnode.opaque_ranks comps in
              let tenv = List.map ranks ~f:label_name in
              let comp_row (lh, c) =
                let kind, ty_s =
                  match c with
                  | Tnode.Sopaque -> ("type", "(opaque — open mints fresh)")
                  | Tnode.Smanifest e ->
                      ("type", "= " ^ Pretty.ty_to_string ~ns:s.ns ~st:s.store ~prec:0 ~tenv e)
                  | Tnode.Sval t ->
                      ("val", ": " ^ Pretty.ty_to_string ~ns:s.ns ~st:s.store ~prec:0 ~tenv t)
                in
                Vdom.Node.div
                  ~attrs:[ Vdom.Attr.class_ "impl-row" ]
                  [
                    Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "open-field-kind" ] [ Vdom.Node.text kind ];
                    Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "chip chip-name" ] [ Vdom.Node.text (label_name lh) ];
                    Vdom.Node.span ~attrs:[ Vdom.Attr.class_ "feedback-type" ] [ Vdom.Node.text (" " ^ ty_s) ];
                  ]
              in
              [
                Vdom.Node.div [ Vdom.Node.text "signature (translucent: opaque + manifest type members)" ];
                Vdom.Node.h3
                  ~attrs:[ Vdom.Attr.class_ "section-title" ]
                  [ Vdom.Node.text (Printf.sprintf "components (%d)" (List.length comps)) ];
                Vdom.Node.div ~attrs:[ Vdom.Attr.class_ "impl-list" ] (List.map comps ~f:comp_row);
              ]
          ...
          | Some (Definition.Term node) ->
              ...
              in
              (* a module (sig-typed term) can be opened into the namespace from here *)
              let open_row =
                if not (Ops.is_module_term s h) then Vdom.Node.none
                else
                  Vdom.Node.div
                    ...
                    [
                      Vdom.Node.div
                        ~attrs:[ Vdom.Attr.class_ "detail-note" ]
                        [ Vdom.Node.text "module — open its type members into the namespace:" ];
                      Vdom.Node.input
                        ~attrs:
                          [
