/* p16 substrate tests: primitive layer (Hash, Mint) + the design/12 Counter
...
/* p16: generative `open` of an existential yields a fresh abstract type
   (extracted witness) and the package over it; two opens give distinct abstract
   types; the opened ops observe the same value. Plus lists + fold. */
let test_open_and_lists = () => {
  let st = Store.create();
  let ns = Namespace.create();
  let ms = Mint.make_source();
  let res = s =>
  ...
  let ex = "exists t. t * ((t -> t) * (t -> Int))";
  let pkg = res("pack [Int] (0, (\\x: Int. x + 1, \\x: Int. x)) as " ++ ex);

  /* open generatively with a fresh mint */
  let m1 = Mint.fresh(ms);
  let o1 = unwrap(Store.ingest_term(st, Node.Open({pkg, mint: m1})));
  let at1 = Store.ingest_type(st, Tnode.Abstract(m1));
  switch (Store.find(st, tyof_in(st, o1))) {
  | Some(Definition.Type(Tnode.Product(a, _))) =>
    Alcotest.(check(bool))("opened value : <extracted> * ...", true, Hash.equal(a, at1))
  | _ => Alcotest.fail("open result is not a product over the extracted type")
  };

  /* use the opened package positionally: get (incr (incr empty)) = 2 */
  let r = Node.Ref(o1);
  let empty = Node.Fst(r);
  let incr = Node.Fst(Node.Snd(r));
  let get = Node.Snd(Node.Snd(r));
  let use = Node.App(get, Node.App(incr, Node.App(incr, empty)));
  let use_h = unwrap(Store.ingest_term(st, use));
  Alcotest.(check(bool))("observation is Int", true, Hash.equal(tyof_in(st, use_h), Store.int_type(st)));
  switch (Eval.eval_top(st, Node.Ref(use_h))) {
  | Ok(Eval.VInt(2)) => Alcotest.(check(bool))("opened counter observes 2", true, true)
  | Ok(v) => Alcotest.fail("unexpected: " ++ Eval.to_string(v))
  | Error(m) => Alcotest.fail("stuck: " ++ m)
  };

  /* a second open mints a distinct abstract type */
  let m2 = Mint.fresh(ms);
  let _o2 = unwrap(Store.ingest_term(st, Node.Open({pkg, mint: m2})));
  let at2 = Store.ingest_type(st, Tnode.Abstract(m2));
  Alcotest.(check(bool))("two opens give distinct abstract types", false, Hash.equal(at1, at2));

  /* lists: fold (+) over [1,2,3] = 6 */
  let sum =
...
let test_list_literals = () => {
  ...
  /* head must be synthesizable in the empty context */
  switch (resolve("\\x: Int. [| x |]")) {
  | Error(_) => Alcotest.(check(bool))("locally-bound head rejected", true, true)
  | Ok(_) => Alcotest.fail("expected element-type inference to fail under a binder")
  };
