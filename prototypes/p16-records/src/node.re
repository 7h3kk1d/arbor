...
type t =
  ...
  | TyLam(t)
  | TyApp(t, Hash.t)
  /* Existential (p14): pack hides a witness type behind `ty` (an Exists type);
     unpack binds a fresh type variable and a value variable in its body. */
  | Pack({
      witness: Hash.t,
      body: t,
      ty: Hash.t,
  ...
  | Unpack(t, t)
  /* p16: `Open{pkg, mint}` opens an existential generatively — its type is the
     package body over a fresh Abstract(mint). Lists: Nil(elem-ty), Cons(h, t),
     Fold(list, init, step) — a right fold (the only bounded iterator). */
  | Open({
      pkg: t,
      mint: Mint.t,
    })
