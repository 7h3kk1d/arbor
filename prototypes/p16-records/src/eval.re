...
let rec eval = (st: Store.t, env: list(value), node: Node.t): value =>
  ...
  | Node.Pack({body, _}) => eval(st, env, body) /* the witness type erases */
  | Node.Unpack(scrut, body) =>
    /* at runtime, unpack is just a let-binding of the packed value */
    let v = eval(st, env, scrut);
    eval(st, [v, ...env], body);
  | Node.Open({pkg, _}) => eval(st, env, pkg) /* the extracted type erases */
  | Node.Nil(_) => VNil
