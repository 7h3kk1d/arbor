/* Attachment — the substrate's aspect store. Carries from p4 verbatim
   except that the aspect_value sum gains a Translation_target(Hash.t)
   variant so translators can register their outputs here alongside
   evaluator results.

   p5 registers three descriptors at REPL init:
     - arith:eval        (arith evaluator cache)
     - lc:eval           (lc evaluator cache)
     - translation-to-lc (translator cache; source-indexed)
   ...
   must not be cached — see lc_eval.re for the explicit skip. */
