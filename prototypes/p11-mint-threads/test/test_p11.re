...
let make_substrate = () => {
  ...
  Attachment.register_descriptor(att, Follow_clean.descriptor);
...
/* ==================== p11: mint threads, history, update strategies ==================== */

/* Ingest source as a top-level named-term: ingest body via Resolver,
   wrap with a fresh mint, bind under `name`. Returns the named hash. */
let bind_term_fresh = (~ns, ~store, ~att, ~name, src: string): Hash.t => {
  let r = must_ingest(~ns, ~store, ~att, src);
  let named = Store.register_named_term(store, r.hash);
  Namespace.bind(ns, ~name, named);
  named;
};

let test_edit_of_preserves_mint = () => {
  let (store, att, ns) = make_substrate();
  let v1 = bind_term_fresh(~ns, ~store, ~att, ~name="math.add", "\\x: Int => \\y: Int => x + y");
  let m1 = Store.mint_of(store, v1);
  /* Edit-of: produce a new body, wrap with the same mint. */
  let new_r = must_ingest(~ns, ~store, ~att, "\\x: Int => \\y: Int => y + x");
  let v2 =
  ...
  Alcotest.(check(neg(string)))("distinct hashes", v1, v2);
  Alcotest.(check(bool))(
    "same mint",
    true,
    Store.mint_of(store, v2) == m1,
  );
  let thread = Store.thread_of(store, v1);
  Alcotest.(check(int))("thread has 2 entries", 2, List.length(thread));
  Alcotest.(check(bool))(
    "thread contains v1 and v2",
    true,
    List.exists(h => h == v1, thread) && List.exists(h => h == v2, thread),
  );
};

let test_callers_of_named_body = () => {
  let (store, att, ns) = make_substrate();
  let add = bind_term_fresh(~ns, ~store, ~att, ~name="math.add", "\\x: Int => \\y: Int => x + y");
  let add_body = Store.unwrap_named(store, add);
  let parents = Store.callers_of(store, add_body);
  Alcotest.(check(bool))(
    "named wrapper appears as caller of its body",
    true,
    List.exists(h => h == add, parents),
  );
};

let test_callers_closure_through_inlining = () => {
  let (store, att, ns) = make_substrate();
  let add = bind_term_fresh(~ns, ~store, ~att, ~name="math.add", "\\x: Int => \\y: Int => x + y");
  let add_body = Store.unwrap_named(store, add);
  let _inc =
  ...
  let closure = Store.callers_closure(store, add_body);
  /* inc's body inlines add's body — `closure` should contain at
     least add (named wrapper) plus some node referencing add_body. */
  Alcotest.(check(bool))(
    "closure non-empty",
    true,
    List.length(closure) > 0,
  );
  /* add (named wrapper) is in the closure */
  Alcotest.(check(bool))(
    "add named is in closure",
    true,
    List.exists(h => h == add, closure),
  );
};

let test_binding_history_grows = () => {
  let (store, att, ns) = make_substrate();
  let h1 = bind_term_fresh(~ns, ~store, ~att, ~name="pi", "3");
  let r2 = must_ingest(~ns, ~store, ~att, "314");
  let h2 = Store.register_named_term(store, r2.hash);
  Namespace.rebind(ns, ~name="pi", h2);
  let hist = Namespace.history(ns, "pi");
  Alcotest.(check(int))("two events", 2, List.length(hist));
  switch (hist) {
  | [(Some(top), _), (Some(prev), _)] =>
    Alcotest.(check(string))("head is rebind", h2, top);
    Alcotest.(check(string))("tail is original bind", h1, prev);
  | _ => Alcotest.fail("unexpected history shape")
  };
};

let test_unbind_appends_tombstone = () => {
  let (store, att, ns) = make_substrate();
  let _ = bind_term_fresh(~ns, ~store, ~att, ~name="pi", "3");
  let _: bool = Namespace.unbind(ns, ~name="pi");
  switch (Namespace.history(ns, "pi")) {
  | [(None, _), (Some(_), _)] => ()
  | _ => Alcotest.fail("expected None head after unbind")
  };
};

let test_reverse_lookup_with_version = () => {
  let (store, att, ns) = make_substrate();
  let v1 = bind_term_fresh(~ns, ~store, ~att, ~name="pi", "3");
  let r2 = must_ingest(~ns, ~store, ~att, "314");
  let v2 = Store.register_named_term(store, r2.hash);
  Namespace.rebind(ns, ~name="pi", v2);
  switch (Namespace.reverse_lookup_with_version(ns, v1)) {
  | Some(("pi", 1, false)) => ()
  | _ => Alcotest.fail("expected v1 orphan to be pi v1")
  };
  switch (Namespace.reverse_lookup_with_version(ns, v2)) {
  | Some(("pi", 2, true)) => ()
  | _ => Alcotest.fail("expected v2 to be current pi v2")
  };
};

let test_pretty_renders_orphan_with_version = () => {
  let (store, att, ns) = make_substrate();
  let v1 = bind_term_fresh(~ns, ~store, ~att, ~name="pi", "3");
  let r2 = must_ingest(~ns, ~store, ~att, "314");
  let v2 = Store.register_named_term(store, r2.hash);
  Namespace.rebind(ns, ~name="pi", v2);
  switch (Pretty.display_name(ns, v1)) {
  | Some(name) =>
    Alcotest.(check(string))("orphan name", "pi(v1)", name)
  | None => Alcotest.fail("expected display_name for orphan")
  };
};

let test_multi_rebind_atomic = () => {
  let (store, att, ns) = make_substrate();
  let h1 = bind_term_fresh(~ns, ~store, ~att, ~name="a", "1");
  let h2 = bind_term_fresh(~ns, ~store, ~att, ~name="b", "2");
  let bogus = "DEADBEEF";
  let target_exists = h => Store.has(store, h);
  switch (
    Namespace.multi_rebind(
      ns,
      ~target_exists,
      [("a", h2), ("b", bogus)],
    )
  ) {
  | Error(Namespace.Mr_target_missing("b", _)) => ()
  | _ => Alcotest.fail("expected validation failure")
  };
  Alcotest.(check(option(string)))(
    "a unchanged",
    Some(h1),
    Namespace.resolve(ns, "a"),
  );
  Alcotest.(check(option(string)))(
    "b unchanged",
    Some(h2),
    Namespace.resolve(ns, "b"),
  );
};

let test_strategy_pin_orphans_old = () => {
  let (store, att, ns) = make_substrate();
  let old_h =
    bind_term_fresh(~ns, ~store, ~att, ~name="math.inc", "\\n: Int => n + 1");
  let new_r = must_ingest(~ns, ~store, ~att, "\\n: Int => n + 2");
  switch (
    Update_strategy.apply_edit(
      ~ns, ~store, ~att, ~name="math.inc",
      ~old_named=old_h, ~new_body=new_r.hash, ~strategy=Pin,
    )
  ) {
  | Ok({new_named, rebinds}) =>
    Alcotest.(check(int))("only one rebind", 1, List.length(rebinds));
    Alcotest.(check(option(string)))(
      "math.inc points at new",
      Some(new_named),
      Namespace.resolve(ns, "math.inc"),
    );
    Alcotest.(check(bool))(
      "old is orphan",
      true,
      Namespace.names_of(ns, old_h) == [],
    );
    Alcotest.(check(bool))(
      "old and new share mint",
      true,
      Store.mint_of(store, old_h) == Store.mint_of(store, new_named),
    );
  | Error(e) =>
    Alcotest.failf("pin should succeed: %s", Update_strategy.error_to_string(e))
  };
};

let test_strategy_follow_cascades = () => {
  let (store, att, ns) = make_substrate();
  let add =
    bind_term_fresh(
      ~ns, ~store, ~att,
  ...
  /* Edit math.add: same type, body permuted. Follow should cascade
     and rebuild math.inc with the new body. */
  let new_r =
    must_ingest(~ns, ~store, ~att, "\\x: Int => \\y: Int => y + x");
  switch (
    Update_strategy.apply_edit(
      ~ns, ~store, ~att,
      ~name="math.add", ~old_named=add,
      ~new_body=new_r.hash, ~strategy=Follow,
    )
  ) {
  | Ok({rebinds, _}) =>
    /* At minimum math.add and math.inc are rebound. */
    Alcotest.(check(bool))(
      "math.add in rebinds",
      true,
      List.exists(((n, _)) => n == "math.add", rebinds),
    );
    Alcotest.(check(bool))(
      "math.inc in rebinds",
      true,
      List.exists(((n, _)) => n == "math.inc", rebinds),
    );
    /* New math.inc preserves its own mint. */
    let new_inc = Option.get(Namespace.resolve(ns, "math.inc"));
    Alcotest.(check(bool))(
      "math.inc mint preserved",
      true,
      Store.mint_of(store, new_inc) == Store.mint_of(store, inc),
    );
  | Error(e) =>
    Alcotest.failf("follow should succeed: %s", Update_strategy.error_to_string(e))
  };
};

let test_follow_clean_type_preserving_short_circuit = () => {
  let (store, att, ns) = make_substrate();
  let h1 = bind_term_fresh(~ns, ~store, ~att, ~name="f", "\\x: Int => x + 1");
  let r2 = must_ingest(~ns, ~store, ~att, "\\x: Int => x + 2");
  let h2 = Store.register_named_term(store, r2.hash);
  let result = Follow_clean.compute(~store, ~att, ~h_old=h1, ~h_new=h2);
  Alcotest.(check(bool))("type-preserving edit is clean", true, result);
};

let test_follow_clean_cached = () => {
  let (store, att, ns) = make_substrate();
  let h1 = bind_term_fresh(~ns, ~store, ~att, ~name="f", "\\x: Int => x");
  let r2 = must_ingest(~ns, ~store, ~att, "\\x: Int => x + 0");
  let h2 = Store.register_named_term(store, r2.hash);
  let _ = Follow_clean.compute(~store, ~att, ~h_old=h1, ~h_new=h2);
  let stats_before = Attachment.stats(att);
  let _ = Follow_clean.compute(~store, ~att, ~h_old=h1, ~h_new=h2);
  let stats_after = Attachment.stats(att);
  /* The cache check is via `peek` not `get`, so hits don't bump
     counters; what we verify is just that repeated compute yields
     the same answer without raising. */
  Alcotest.(check(int))(
    "entries did not grow",
    stats_before.entries,
    stats_after.entries,
  );
};

let test_strategy_migrate_partial = () => {
  let (store, att, ns) = make_substrate();
  let add =
    bind_term_fresh(
      ~ns, ~store, ~att,
  ...
  let new_r =
    must_ingest(~ns, ~store, ~att, "\\x: Int => \\y: Int => y + x");
  /* Migrate only inc; dec stays pinned. */
  switch (
    Update_strategy.apply_edit(
      ~ns, ~store, ~att,
      ~name="math.add", ~old_named=add,
      ~new_body=new_r.hash, ~strategy=Migrate([inc]),
    )
  ) {
  | Ok({rebinds, _}) =>
    Alcotest.(check(bool))(
    ...
    Alcotest.(check(bool))(
      "math.dec NOT in rebinds",
      false,
      List.exists(((n, _)) => n == "math.dec", rebinds),
    );
    /* dec still points at its original hash. */
    Alcotest.(check(option(string)))(
      "dec unchanged",
      Some(dec),
      Namespace.resolve(ns, "math.dec"),
    );
  | Error(e) =>
    Alcotest.failf(
      "migrate should succeed: %s",
      Update_strategy.error_to_string(e),
    )
  };
};
  ...
  Alcotest.run(
    ...
    [
      ...
      (
        ...
        [
          Alcotest.test_case(
            "edit-of preserves mint",
            `Quick,
            test_edit_of_preserves_mint,
          ),
          Alcotest.test_case(
            "callers_of named body",
            `Quick,
            test_callers_of_named_body,
          ),
          Alcotest.test_case(
            "callers_closure through inlining",
            `Quick,
            test_callers_closure_through_inlining,
          ),
          Alcotest.test_case(
            "binding history grows",
            `Quick,
            test_binding_history_grows,
          ),
          Alcotest.test_case(
            "unbind appends tombstone",
            `Quick,
            test_unbind_appends_tombstone,
          ),
          Alcotest.test_case(
            "reverse_lookup_with_version",
            `Quick,
            test_reverse_lookup_with_version,
          ),
          Alcotest.test_case(
            "Pretty.display_name renders orphan vN",
            `Quick,
            test_pretty_renders_orphan_with_version,
          ),
          Alcotest.test_case(
            "multi_rebind atomic on validation failure",
            `Quick,
            test_multi_rebind_atomic,
          ),
          Alcotest.test_case(
            "Pin orphans old hash",
            `Quick,
            test_strategy_pin_orphans_old,
          ),
          Alcotest.test_case(
            "Follow cascades to callers",
            `Quick,
            test_strategy_follow_cascades,
          ),
          Alcotest.test_case(
            "follow-clean type-preserving short-circuits",
            `Quick,
            test_follow_clean_type_preserving_short_circuit,
          ),
          Alcotest.test_case(
            "follow-clean cached",
            `Quick,
            test_follow_clean_cached,
          ),
          Alcotest.test_case(
            "Migrate updates only chosen sites",
            `Quick,
            test_strategy_migrate_partial,
          ),
        ],
      ),
      (
