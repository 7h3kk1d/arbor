/* Attachment — the substrate's aspect store. Holds derived and
   ...
   This prototype registers one descriptor: the λ-calculus eval cache.
   The API shape matches docs/design/06-architecture.md:34-42. The
   aspect_value variant gains a Eval_step_limit case that the evaluator
   uses to report a bounded timeout; callers (see Eval) must not cache
   that variant because it is a *partial* result — a future call with a
   higher step budget may succeed. */

type aspect_id = string; /* e.g. "lc:eval" */
type procedure_id = string; /* e.g. "lc:eval:v1" */
...
/* Passive lookup — no hit/miss accounting. */
