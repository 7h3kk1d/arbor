...
/* Strip an input prefix marker ("#" or legacy "h:") from a user-supplied
   string. Returns the bare hex portion. */
...
/* True if `s` looks like a hash reference: either carries the display
   prefix, or is all lowercase-hex and at least four characters (arbitrary
   threshold chosen to avoid false positives on short identifiers). */
