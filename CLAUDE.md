...
- Single typed language: native `Int`, `Bool`, `String`, `Product`; annotated lambdas (p6-style); monomorphic `let`; `if`/`then`/`else`; pairs with `fst`/`snd`; primitive operations `+ - mul / mod && || not ++ ==`. `mul` is a keyword because `*` is the product-type constructor. Types: `Ty.t = Int | Bool | String | Arrow | Product` with stable tag bytes 0x10..0x14 in node encoding. No lc, no stlc, no Church encoding. `Definition.t = Term(Node.t) | Type(Ty.t)` — types are first-class definitions in the same Store, disambiguated by the leading byte (`'P'` for term nodes, `'T'` for type encodings).
...
- **Holes from p8 + permissive bidirectional checker.** Lexer is total; Menhir incremental-API recovery (`parse_recover.re`) is cloned verbatim from p8 and parameterized over the start symbol so the same driver services term and type panes. Grammar adds partial-form productions for truncated `let`, `if`, and lambda. Hole tag byte 0x0d (under language tag 'P'). `Typecheck.check_top` returns one of `Well_typed(ty)`, `Well_typed_with_holes(ty)`, or `Ill_typed(msg)`. Hard mismatches (`1 + true`, `(\x:Int. x) "abc"`) reject ingest; holey-but-consistent terms ingest with the `Type_with_holes(Hash.t)` aspect cached (the hash references a `Definition.Type`). Hole rules: `check ctx Hole expected → Ok` (accepts any expected type); `synth ctx Hole → Int` as best-guess. No unification yet — the planned escalation if too many sensible programs reject.
...
- Interface: Bonsai + js_of_ocaml three-pane web app (no server). Left pane is a collapsible namespace tree built per render from the flat dotted-string namespace; leaves show a kind-badge (`T` for type definitions, none for terms), the leaf segment, short hash, and ◌ badge if `has-holes:v1` is true. Center pane has a term/type mode toggle above one textarea, recovered-AST panel (term mode only), and feedback area; each mode keeps its own buffer and bind-as input. Right pane is the selected hash's detail view — type definitions render structurally without aspect rows, terms keep the typecheck/has-holes/eval rows. Bootstrap seeds: type aliases `alias.IntPair`/`IntEndo`/`BinOp`; terms `math.add`/`sub`/`mul`/`inc`/`apply` (where `math.apply = \f: IntEndo. \x: Int. f x` exercises named-alias resolution), `vector.add` (collides on suffix `add` for ambiguity demo), `string.greet`, `logic.implies`, and `draft.todo` (holey, demos the has-holes badge). Plus built-in primitives `string.length`/`reverse`/`substring`, `int.abs`/`to_string` registered via `Primitive_registry`.

- **Primitive registry** (`Primitive_registry`, `Primitives`): a substrate extension point for built-in functions that have a declared `Ty.t` and an OCaml impl, but no dedicated surface syntax. Each primitive has a stable id (`string:reverse:v1`); the bootstrap builds a wrapping curried Lam whose body is a saturated `Ast.Prim_call(id, args)`, ingests it, and binds it to a namespace name. The user calls primitives via the namespace (`string.reverse "abc"`); the Resolver inlines the wrapping Lam like any other named binding. Adding a new primitive is one entry in `src/primitives.re` — no parser, lexer, or grammar changes per primitive. Per-prim hash stability comes from the id being included in the `Node.Prim_call` encoding; behavior changes require bumping the version in the id (per `docs/design/03-content-addressing.md` §"Primitive identity"). Pretty-printer renders `Prim_call(id, args)` as `#<id> args...` for inspection — display-only, intentionally not re-parseable.
