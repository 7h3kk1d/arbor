...
- **No peer-reviewed publication exists for Unison.** Zero DBLP hits. Its design claims — hashes as identity, names as metadata, never-invalidated caches, no diamond-dependency problem — have never been formally stated, let alone proven. `formalism/`'s theorems appear to be the first formal treatment of this model, which is arbor's largest single citation opportunity. **The source was read on 2026-08-07** (pinned to `db60ce2`) precisely because of this: with no paper, the source is the citable artifact. See `../systems/unison/`. The finding stands unchanged — a source read supplies evidence, not a specification — and the read sharpens the opportunity: the never-invalidated cache is implemented by *omitting* invalidation, and `update`'s correctness rests on a print-then-reparse property that Unison maintains with a golden-file corpus and `formalism/paper/arbor-core.tex` proves as `prop:print-elab`.
...
- **Not a reading plan.** The priority list is ranked by expected impact on what arbor writes down, not sequenced against the roadmap in `../design/09-roadmap.md`. The sequenced reading plan — a spine, tracks keyed to the paper plan, and jump points — lives in `reading-plan.md` *(added 2026-08-06)*.
- **Not source reads.** Several systems here have no publication at all, so the only citable artifact is their implementation. Those live in `../systems/` *(added 2026-08-07)*, pinned to a commit rather than cited to a DOI, with their own evidence tags. First read: `../systems/unison/`. Where a system appears in both places, the theme file says what it is in the bibliography and points; the source read says what the code does.
