...
- **Extrinsic identifiers, and the word "mint."** The persistent-identifier world uses *mint* as its literal verb — ARKs are minted by a Name Assigning Authority, DOIs via DataCite and Crossref. Read against the **intrinsic vs. extrinsic** distinction that Software Heritage draws (a hash you can recompute versus an identifier an authority assigns), a mint mark is best characterized as *an extrinsic identifier smuggled inside an intrinsic one*: it is the one part of the hash that cannot be recomputed from the definition, only read back out. That is a sharper statement of what minting does than "an opt-in axis of identity orthogonal to content addressing," and it explains the cost — see the early-cutoff note in `../related-work/01-content-addressing.md` §"Minting", where a minted definition cannot share derived aspects with its structural twin and so gets type-checked and evaluated twice.
