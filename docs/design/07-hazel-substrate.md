...
- **propl24.** Outlines the *computational commons* vision. Primary source for the substrate-level requirements this project is trying to satisfy. ([hazel.org](https://hazel.org/)) Full citation (added 2026-08-03): **Bandukwala, Blinn & Omar, "Toward a Live, Rich, Composable, and Collaborative Planetary Compute Engine," PROPL 2024** (Programming for the Planet, co-located with POPL), London, January 2024.
- **propl25.** A second, later commons paper worth tracking alongside propl24: **Omar, Coblenz & Madhavapeddy, "A FAIR Case for a Live Computational Commons," PROPL 2025**, pp. 8–13, DOI [10.1145/3759536.3763802](https://doi.org/10.1145/3759536.3763802). Proposes *Fairground* and frames the commons around the FAIR principles — findable, accessible, interoperable, reusable — which is the most directly usable rubric for the requirements checklist below. Note it reaches the commons by DOIs, ORCIDs, and a version-control platform, with **no content-addressing component anywhere in its 53 references**; that gap is what this substrate is for.
...
- **Back-reference (2026-08-03).** Per the contract above, the citations for the research lines below — all fourteen Hazel papers with authors, venues, and DOIs — now live in `../related-work/02-structure-editors.md`, and the label/module/existential material the type-abstraction line leans on lives in `../related-work/03-modules-abstraction.md`. That directory is backward-looking (what arbor's existing claims sit next to) and does not displace the per-topic policy stated in this section: it changes nothing about *when* a paper gets read or a technique adopted. Two findings from it bear directly on lines below — the POPL 2024 totality result versus arbor's ingest-time rejection of ill-typed terms (research line #5, #9), and Grove's minted vertex UIDs versus content hashes (research line #10) — both filed in `open-questions.md` §"Related work".
