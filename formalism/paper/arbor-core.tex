...
\end{theorem}
\begin{proof}[Proof sketch]
\textsc{Bind}/\textsc{Rebind}/\textsc{Unbind}/\textsc{Eval} leave $\Store$ fixed. $\ingest$
and the migration rewrite only add entries, and each key is $\hashof{n}$ for the very node $n$
stored, so any collision on a key is with an identical node ($\star$). \qed
\end{proof}
...
$\reconstruct$ is defined on every new entry. The migration rewrite preserves both by
construction (\cref{rem:gate}). Name edits do not touch $\Store$.
\emph{$\wf$ (iii):} no existing node changes (\cref{thm:mono}), and every newly added node's
references point at pre-existing closed entries (\textsc{Ingest} premise) or at rewrites whose
registration precedes their referrers' (\textsc{Migrate}); ordering new nodes by registration
therefore extends any topological order of the old reference graph, so no cycle is created.
