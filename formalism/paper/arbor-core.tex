...
substrate state:
\begin{itemize}[nosep]
  \item a content-addressed \textbf{term store} $\Store$ together with its set $\Roots$ of
  \textbf{definition roots} (\cref{sec:store});
  ...
  \item an \textbf{edit calculus} of transitions on
  $\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist}$, where $\Hist$ is an append-only binding
  history (\cref{sec:edit}).
\end{itemize}
...
\begin{definition}[Definition roots]\label{def:roots}
\src{p11 definition.re: Named\_term / Named\_type} A \emph{rooted store} is a pair
$(\Store, \Roots)$ with $\Roots \subseteq \dom(\Store)$. The hashes in $\Roots$ are the
\emph{definitions}: complete top-level terms, registered as such by the edit calculus
(\textsc{Ingest} and \textsc{Migrate}, \cref{sec:edit}). Every other entry of $\Store$ is an
anonymous subterm node --- shared structure that need not be closed. Because storage is shallow,
$\dom(\Store)$ inevitably contains such nodes (a bare $\nVar{1}$ under some $\nLam{}$ is an
entry); $\Roots$ is what lets the metatheory quantify over \emph{definitions} without
quantifying over every shared node. \texttt{p11} realizes the same distinction as a sort marker
in the store --- the \texttt{Named\_term}/\texttt{Named\_type} wrappers, whose mint this
artifact drops; \texttt{p4} leaves it implicit (a definition is whatever a name has been bound
to).
\end{definition}
...
\src{03-content-addressing.md: ``hashes never dangle''} A rooted store is \emph{well-formed},
$\wf(\Store,\Roots)$, iff:
\begin{enumerate}[nosep,label=(\roman*)]
  \item every $h \in \dom(\Store)$ has $\reconstruct(\Store,h)$ defined (every structural child
  resolves --- no dangle);
  \item every $h \in \dom(\Store)$ has $\refs(\Store,h) \subseteq \Roots$ (references cite
  definitions, never anonymous subterm nodes);
  \item every $d \in \Roots$ has $\isclosed{\reconstruct(\Store,d)}$ (definitions are closed ---
  the load-bearing \texttt{p4} invariant that, with (ii), lets a reference unfold with no
  shifting);
...
Clause (iv) is not implied by the others. $\reconstruct$ stops at $\tRef{}$ leaves, so clause
...
that is, $\hashof{\nRef{h}} = h$ --- satisfies (i)--(iii) with $h \in \Roots$, because
injectivity ($\star$) alone is consistent with such a fixpoint. A concrete hash function makes
the fixpoint unforgeable (one would need $h$ before computing it), but the abstract model must
exclude it explicitly. Clause (iv) is what makes the callers relation well-founded, so that
migration's dependency order exists (\cref{def:deporder}) and its cascade terminates; evaluation
does not need it (\textsc{E-Ref} on a cyclic reference simply diverges).
\end{remark}
...
If $\wf(\Store,\Roots)$ and $\isclosed{t}$ then no evaluation position reached from $t$ has a
free $\Var{}$ at its head; hence the missing $\Var$-rule loses no closed-term behavior.
\end{lemma}
\begin{proof}[Proof sketch]
$\mathsf{beta}$ preserves closedness (\cref{def:closed}, \cref{def:beta}), and so does
reference unfolding: an unfolded target is in $\Roots$ by $\wf$ (ii), hence closed by $\wf$
(iii). A closed weak-head normal form has an abstraction at its head. (Without the $\wf$
premise the lemma is false --- unfolding a reference into an \emph{open} stored term would
smuggle in a free variable.) \qed
\end{proof}
...
$C = \Config{\Store}{\Roots}{\Names}{\Cache}{\Hist}$. $C$ is \emph{coherent} iff:
\begin{enumerate}[nosep,label=(\alph*)]
  \item $\wf(\Store,\Roots)$ (\cref{def:wf});
  \item $\ran(\Names) \subseteq \Roots$ (every bound name denotes a definition);
  \item every hash in a $\mathsf{Some}$ event of $\Hist$ lies in $\Roots$ (orphans remain
  closed, reconstructible definitions --- what makes \cref{def:orphan}'s rendering meaningful);
...
\begin{mathpar}
  \inferrule*[right=Ingest]
    {\isclosed{t} \\ \refs(t) \subseteq \Roots \\ \ingest(\Store,t) = (\Store',h)}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store'}{\Roots \cup \{h\}}{\Names}{\Cache}{\Hist}}
  \and
  \inferrule*[right=Eval]{\evals{\Store}{\tRef{h}}{\val} \\ h \notin \dom(\Cache)}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store}{\Roots}{\Names}{\upd{\Cache}{h}{\val}}{\Hist}}
  \\
  \inferrule*[right=Bind]{x \notin \dom(\Names) \\ h \in \Roots}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store}{\Roots}{\upd{\Names}{x}{h}}{\Cache}{\upd{\Hist}{x}{(\mathsf{Some}\,h,\tau){:}\Hist(x)}}}
  \and
  \inferrule*[right=Rebind]{h \in \Roots}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store}{\Roots}{\upd{\Names}{x}{h}}{\Cache}{\upd{\Hist}{x}{(\mathsf{Some}\,h,\tau){:}\Hist(x)}}}
  \\
  \inferrule*[right=Unbind]{x \in \dom(\Names)}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store}{\Roots}{\Names\setminus x}{\Cache}{\upd{\Hist}{x}{(\bot,\tau){:}\Hist(x)}}}
\end{mathpar}
$\bind$, $\rebind$, $\unbind$ touch only $\Names$ and $\Hist$; $\Store$ and $\Roots$ are
untouched. Renaming is derived as $\unbind$ then $\bind$ \src{namespace.re: rename}.
\end{definition}
...
deliberate. \textsc{Ingest} requires a closed term whose references cite existing definitions:
these are facts the editing layer establishes by construction (\cref{lem:elab-premises}), but
they must be \emph{premises} of the transition or coherence preservation (\cref{thm:wf}) fails
--- ingesting $\tRef{h_0}$ for an unregistered $h_0$ would dangle, and an open definition would
break reference unfolding (\cref{lem:closed-no-stuck}). \textsc{Bind}/\textsc{Rebind} require
$h \in \Roots$: the namespace layer alone cannot check this (\texttt{p11}'s
\texttt{namespace.re} accepts any hash), so the prototypes validate it one level up ---
$\multirebind$'s \texttt{target\_exists} predicate \src{namespace.re: multi\_rebind} and the
editor's resolution step; a transition on the whole configuration can state it directly.
\textsc{Rebind} keeps no premise on $x$: like \texttt{p11}'s \texttt{rebind}, it also binds
fresh names, so \textsc{Bind} is its first-bind special case, kept for readability and for the
...
$\callers(\Store,\Roots,h) \defeq \{\, g \in \Roots \mid h \in \refs(\Store,g)\,\}$;
$\callersstar(\Store,\Roots,h)$ is its transitive closure (definitions reaching $h$ through a
chain of references). Callers range over $\Roots$, not $\dom(\Store)$: the store also holds
every anonymous subterm node, and ``rewriting'' those independently of the definitions
containing them would be meaningless. \texttt{p11} makes the same restriction one call later
--- its caller index tracks \emph{all} structural parents, and the cascade filters to
\texttt{Named} definitions \src{update\_strategy.re: cascade\_pairs}.
\end{definition}
...
For finite $K \subseteq \Roots$, a \emph{dependency order} on $K$ is a linear order
...
callees before callers. By $\wf$ clause (iv) a dependency order always exists (any topological
sort of the acyclic reference graph restricted to $K$); without (iv) it need not
(\cref{rem:acyclic}).
\end{definition}

\begin{definition}[Cascade]\label{def:cascade}
\src{update\_strategy.re: cascade\_pairs} Given an edit of definition
$g_{\mathrm{old}} \in \Roots$ to a new body ingested at $g_{\mathrm{new}}$ (satisfying the
\textsc{Ingest} premises), and a filter $p$, the cascade fixes a dependency order
(\cref{def:deporder}) on $\callersstar(\Store,\Roots,g_{\mathrm{old}})$ and processes it in
that order, accumulating a rewrite map $\rho$ initialized to
$\{g_{\mathrm{old}}\mapsto g_{\mathrm{new}}\}$. For each caller $g$ with $p(g)$: let $g'$ be
the hash of $\ingest(\substrefs{\rho}{\reconstruct(\Store,g)})$; if $\substrefs{\rho}{\cdot}$
changed the body, require the \textbf{gate} to hold of the rewritten definition
(\cref{rem:gate}); on success extend $\rho$ with $g \mapsto g'$ and emit the pair $(g,g')$. The
cascade \emph{aborts} if any in-scope caller fails the gate. Dependency order is what makes
$\rho$ complete at each step: every in-scope definition that $g$ directly references has
already been processed, so no rewritten caller is left citing a stale sibling. (\texttt{p11}'s
\texttt{callers\_closure} returns DFS \emph{discovery} order, which is not a dependency order;
its cascade tolerates this because \texttt{p11} has no $\nRef{}$ --- references there are deep
structural containment, and its substitution is keyed on $g_{\mathrm{old}}$'s body from the
start. With shallow $\nRef{}$ rewriting, the order is load-bearing.)
\end{definition}

\begin{lemma}[The cascade is order-independent]\label{lem:cascade-order}
The final rewrite map $\rho$, the emitted pairs $P$, and the resulting store extension are the
same for every choice of dependency order.
\end{lemma}
\begin{proof}[Proof sketch]
By $\wf$ (iv) the reference graph restricted to the in-scope callers is well-founded, so the
cascade's result has an order-free characterization by recursion on it: for in-scope $g$,
$\rho(g)$ is the root hash of
$\ingest(\substrefs{\rho \restriction \refs(\Store,g)}{\reconstruct(\Store,g)})$, where
$\rho$ restricted to $\refs(\Store,g)$ is determined by the same recursion on $g$'s in-scope
referents together with the fixed seed $g_{\mathrm{old}} \mapsto g_{\mathrm{new}}$
(out-of-scope referents never enter $\dom(\rho)$). Any dependency-order pass computes exactly
this fixpoint. \qed
\end{proof}
...
ill-typed one \src{update\_strategy.re: Typecheck.check\_top}. Untyped, the gate is exactly the
\textsc{Ingest} premises for the rewritten body: closedness, and references citing definitions
(counting the pairs emitted so far as definitions). Both hold by construction ---
$\substrefs{\rho}{\cdot}$ replaces closed references by closed references, and every rewrite
target is $g_{\mathrm{new}}$ or a previously emitted $g'$. So the gate is \emph{near-total} ---
$\Follow$ essentially always succeeds here. That is the honest finding: the interesting failure
modes of $\Follow$ are introduced by \emph{types}, motivating the typed successor
(\texttt{open-questions.md}).
\end{remark}
...
\]
$\multirebind$ validates $U$ (every target in $\Roots'$) and \textbf{commits all-or-nothing}:
\begin{mathpar}
  \inferrule*[right=Migrate]
    {\text{cascade succeeds}\ \Rightarrow\ (\Store',P) \\
     \Roots' = \Roots \cup \{g_{\mathrm{new}}\} \cup \{\,g' \mid (g,g') \in P\,\} \\
     \multirebind(\Names,\Hist,U) = (\Names',\Hist')}
    {\Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
     \Config{\Store'}{\Roots'}{\Names'}{\Cache}{\Hist'}}
\end{mathpar}
On a cascade or validation failure, $\Names$ and $\Hist$ are untouched; $\Store$ and $\Roots$
may retain the candidate hashes they accumulated (there is no orphan GC; in \texttt{p11} the
candidates are already-registered \texttt{Named} wrappers --- closed definitions, so retaining
them as roots is harmless). Thus \textbf{only $\langle\Names,\Hist\rangle$ is transactional;
$\Store$ and $\Roots$ are accumulate-only}.
\end{definition}
...
If $C = \Config{\Store}{\Roots}{\Names}{\Cache}{\Hist} \steps
\Config{\Store}{\Roots}{\Names'}{\Cache}{\Hist'}$
...
Every transition has $\Store \subseteq \Store'$ (as partial functions) and
$\Roots \subseteq \Roots'$, and no existing entry $h \mapsto n$ is ever overwritten.
\end{theorem}
\begin{proof}[Proof sketch]
\textsc{Bind}/\textsc{Rebind}/\textsc{Unbind}/\textsc{Eval} leave $\Store$ and $\Roots$ fixed.
$\ingest$ and the cascade only add entries, and each key is $\hashof{n}$ for the very node $n$
stored, so any collision on a key is with an identical node ($\star$). \textsc{Ingest} and
\textsc{Migrate} only ever enlarge $\Roots$. \qed
\end{proof}
...
\end{theorem}
\begin{proof}[Proof sketch]
Clause by clause. \emph{$\wf$ (i)--(iii):} \textsc{Ingest}'s premises supply closedness of the
new root and $\refs(t) \subseteq \Roots$; its structural children are registered bottom-up, so
$\reconstruct$ is defined on every new entry. The cascade adds only bodies passing the gate,
which is those same premises (\cref{rem:gate}). Name edits do not touch $\Store,\Roots$.
\emph{$\wf$ (iv):} no existing node changes (\cref{thm:mono}), and every newly added node's
references lie in the pre-existing $\Roots$ (\textsc{Ingest} premise) or in
$\{g_{\mathrm{new}}\} \cup$ previously-emitted rewrites (\textsc{Migrate}); ordering new nodes
by registration therefore extends any topological order of the old reference graph, so no cycle
is created. \emph{(b), (c):} \textsc{Bind}/\textsc{Rebind} add bindings and $\mathsf{Some}$
events only with $h \in \Roots$; $\multirebind$'s targets are validated in $\Roots'$;
\textsc{Unbind} only removes a binding and appends $\bot$. \emph{(d):} \cref{thm:histcoh} and
\cref{cor:cache}. Note where the old hand-wave went: the \textsc{Ingest} case no longer appeals
to ``edit-time resolution'' --- that fact is now \cref{lem:elab-premises}, and here it is simply
a premise. \qed
\end{proof}

\begin{theorem}[Evaluation stability under growth]\label{thm:stability}
Suppose $\wf(\Store,\Roots)$, $\Store \subseteq \Store'$, $\Roots \subseteq \Roots'$, and
$\wf(\Store',\Roots')$. Then for every $h$, $\evals{\Store}{\tRef{h}}{\val}$ implies
$\evals{\Store'}{\tRef{h}}{\val}$ (and, by determinism, the values agree).
\end{theorem}
\begin{proof}[Proof sketch]
...
\begin{enumerate}[nosep,label=(\alph*)]
  ...
  \callersstar(\Store,\Roots,g_{\mathrm{old}})$ with $\mathrm{strat}$'s predicate true whose
  body actually mentions a rewritten reference --- independently of the dependency order chosen
  (\cref{lem:cascade-order}). Under $\Pin$ none are rewritten and $g_{\mathrm{old}}$ becomes an
  orphan; under $\Follow$ all reachable named callers are; under $\Explicit(S)$, those in $S$.
  \item \emph{(Preservation)} $\wf(\Store',\Roots')$ and \cref{thm:nsb} still hold: definitions
  that were \emph{not} rewritten keep their original reference edges, so their meaning is
  unchanged.
\end{enumerate}
\end{theorem}
\begin{proof}[Proof sketch]
...
\begin{itemize}[nosep]
  ...
  $\Roots$ as a finite set; $\wf$ as a decidable predicate (acyclicity of the finite reference
  graph, clause (iv), is decidable) and configuration coherence (\cref{def:config}) likewise.
